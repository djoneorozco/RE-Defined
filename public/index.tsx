<!-- /public/index.html -->
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>RE-DEFINED — Listing Coach</title>
  <link rel="stylesheet" href="../style.css" />
</head>
<body>
  <!-- ✅ Glow-wrapper background -->
  <div class="glow-wrapper">
    <div class="quiz-container">
      <h1>RE-DEFINED</h1>
      <p class="tagline">Your AI Listing Coach — Unlock the Perfect Buyer Archetype</p>

      <!-- ✅ Quiz Form -->
      <section id="customFields">
        <h2>Tell Us About Your Listing</h2>
        <label>City & Neighborhood:
          <input type="text" id="city" placeholder="e.g. San Antonio, Lackland area" />
        </label>
        <label>Listing Price ($):
          <input type="number" id="price" placeholder="e.g. 275000" />
        </label>
        <label>Biggest Selling Challenge:
          <input type="text" id="challenge" placeholder="e.g. needs repairs, unique buyer" />
        </label>
        <button id="startQuizBtn">Find Best Buyer Type</button>
      </section>

      <!-- ✅ Quiz Questions -->
      <section id="quizSection" style="display:none;">
        <h2 id="question">Loading question...</h2>
        <div id="answers" class="answers"></div>
        <div id="progress" class="progress"></div>
      </section>

      <!-- ✅ Branding Footer -->
      <div class="cta">
        <p>Powered by RealtySaSS</p>
      </div>

      <!-- ✅ Fixed Navigation Button -->
      <div style="margin-top: 24px; text-align: center;">
        <a href="home.html">
          <button style="padding: 12px 24px; background-color: #6c00ff; color: white; border: none; border-radius: 8px; font-size: 16px; cursor: pointer;">
            Go to HomeScreen →
          </button>
        </a>
      </div>

    </div>
  </div>

  <script src="../main.js"></script>
</body>
</html>
